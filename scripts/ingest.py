#!/usr/bin/env python3
"""Ingest documents into the configured vector store for the RAG pipeline.
...
def ingest(
    ...
    """Load, chunk, embed, and store documents in the vector store.
    ...
    Args:
        ...
        dry_run: If True, show what would be ingested without writing to vector store.
    ...
    if dry_run:
        console.print("\n[yellow]🔍 Dry run — no data written to vector store.[/]")
        return {
    ...
    console.print(f"\n[bold cyan]🗄️  Vector Store Engine:[/] {settings.retriever.vector_store}")
    ...
    retriever.ingest(chunks, batch_size=100)

    duration = time.time() - start_time

    # ── Step 4: Verify ────────────────────────────────────────────
    stored_count = retriever.count
...
def main():
    parser = argparse.ArgumentParser(
        description="Ingest documents into the vector store for the RAG pipeline.",
