...
- [x] **Day 7: Integration Tests** — Tests for grounded, ungrounded, unanswerable queries, and edge cases. Open PR for Week 2.
...
- [x] **Day 1: Basic CI** — `.github/workflows/ci.yml` (lint + unit tests on every push).
- [x] **Day 2: Eval Gate CI** — `.github/workflows/eval-gate.yml` (smoke test on push, full golden dataset on PR).
- [x] **Day 3: Eval Config** — `eval_config.yaml` (configurable thresholds that CI reads to gate pass/fail).
- [x] **Day 4: Result Storage** — SQLite storage for eval results per commit.
- [x] **Day 5: Trend CLI** — CLI tool to query historical results and show improving/degrading trends.
- [x] **Day 6: Optimization & Docs** — Dependency caching in CI and branch protection documentation in README.
- [x] **Day 7: End-to-End Test** — Test the full pipeline with an intentionally bad PR to confirm the gate blocks it. Open PR for Week 5.
...
- [x] **Day 1: Dashboard Foundation** — `dashboard/app.py` Streamlit layout, loading eval results from SQLite.
- [x] **Day 2: Analytics Charts** — Add charts for hallucination rate, latency trend, cost, and pass/fail history.
- [x] **Day 3: Query Debugger** — View to inspect per-query results (fired guardrails, retry count, critic reasoning).
- [x] **Day 4: Telemetry** — Structured JSON logging across the pipeline (optional LangSmith integration).
- [x] **Day 5: Documentation** — README overhaul (architecture, setup, local run, extending policies).
- [x] **Day 6: Demo Prep** — Prep scenarios: normal query, retry, guardrail trigger, CI block. Cleanup and type hints.
- [x] **Day 7: Launch** — Tag `v1.0.0`, record demo video, final `CHANGELOG.md` and optional Streamlit deploy. Merge final PR.
...
- [x] **Day 1: Qdrant Integration** — Add **Qdrant** (free cloud tier) as a second retriever backend via `VECTOR_STORE=chroma|qdrant`.
- [x] **Day 2: W&B Setup** — Sign up for **Weights & Biases** free tier and configure the project.
- [x] **Day 3: W&B Logging** — Wire W&B logging into `eval/reporter.py` as an additional sink alongside SQLite.
- [x] **Day 4: Metrics Tracking** — Log hallucination rate, relevancy, latency (p50/p95), and cost per query to W&B.
- [x] **Day 5: Verification** — Run `run_eval.py --mode full` and confirm a new W&B run appears with correct metrics.
- [x] **Day 6: Documentation** — Document the trade-off in the README: local ChromaDB for dev/offline, hosted Qdrant for scaling.
- [x] **Day 7: Review & Merge** — Ensure metrics are comparable across 3+ historical runs and open PR for Week 7.
...
- [x] **Day 1: FastAPI Wrapper** — Wrap `graph.py` in a **FastAPI** app with `/query` and `/health` endpoints.
- [x] **Day 2: Dockerization** — Create `Dockerfile` for the API service.
- [x] **Day 3: Docker Compose** — Create `docker-compose.yml` running the API + ChromaDB (or Qdrant) together.
